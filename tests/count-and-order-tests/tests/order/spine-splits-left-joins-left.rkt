...
(define J-6-0 (token-node (token "*v" SPINE-JOIN 6 0) (box-immutable J-7-0)))
(define J-6-1 (token-node (token "*v" SPINE-JOIN 6 1) (box-immutable J-7-0)))
(define NULL-6-2 (token-node (token "*" NULL-INTERPRETATION 6 2) (box-immutable J-7-1)))
...
(define NULL-2-1 (token-node (token "*" NULL-INTERPRETATION 2 1) (box-immutable 4c-3-2)))
...
(check-expect (hgraph->hfile
               (hgraph (root (list (list (leaf (token "**kern" EXCLUSIVE-INTERPRETATION 0 0))
                                            (parent (token "*^" SPINE-SPLIT 1 0)
                                                    ...
                                                    (list (leaf (token "*" NULL-INTERPRETATION 2 1))
