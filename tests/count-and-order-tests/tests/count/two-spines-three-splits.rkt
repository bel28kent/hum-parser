...
(check-expect (path->hfile "../../data/count/two-spines-three-splits.krn")
              (hfile (list (record "**kern\t**kern" TOKEN
                           ...
                           (record "*\t*v\t*v\t*" TOKEN
                                   ...
                                   26)
                           (record "*v\t*v\t*" TOKEN (list (token "*v" SPINE-JOIN 27)
                                                           (token "*v" SPINE-JOIN 27)
                                                           (token "*" NULL-INTERPRETATION 27))
                                   27)
                           (record "==\t==" TOKEN (list (token "==" MEASURE 28)
                                                        (token "==" MEASURE 28))
                                   28)
                           (record "*-\t*-" TOKEN (list (token "*-" SPINE-TERMINATOR 29)
...
(check-expect (ab-hgraph->lolot
              ...
              (list (list (token "**kern" EXCLUSIVE-INTERPRETATION 0)
                    ...
                    (list (token "*-" SPINE-TERMINATOR 29)
                          (token "*-" SPINE-TERMINATOR 29))))
(check-expect (hfile->ab-hgraph (path->hfile "../../data/count/two-spines-three-splits.krn")
                                ab-hgraph)
              (ab-hgraph (root (list (list (leaf (token "**kern" EXCLUSIVE-INTERPRETATION 0))
                                           (leaf (token "*clefG2" CLEF 1))
                                           (leaf (token "*k[]" KEY-SIG 2))
