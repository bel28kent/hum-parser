...
;; Node definitions
(define TERM-9-0 (terminator-node (token "*-" SPINE-TERMINATOR 9 0)))
(define J-8-0 (token-node (token "*v" SPINE-JOIN 8 0) (box-immutable TERM-9-0)))
(define J-8-1 (token-node (token "*v" SPINE-JOIN 8 1) (box-immutable TERM-9-0)))
(define NULL-7-0 (token-node (token "*" NULL-INTERPRETATION 7 0) (box-immutable J-8-0)))
(define J-7-1 (token-node (token "*v" SPINE-JOIN 7 1) (box-immutable J-8-1)))
(define J-7-2 (token-node (token "*v" SPINE-JOIN 7 2) (box-immutable J-8-1)))
(define NULL-6-0 (token-node (token "*" NULL-INTERPRETATION 6 0) (box-immutable NULL-7-0)))
(define NULL-6-1 (token-node (token "*" NULL-INTERPRETATION 6 1) (box-immutable J-7-1)))
(define J-6-2 (token-node (token "*v" SPINE-JOIN 6 2) (box-immutable J-7-2)))
(define J-6-3 (token-node (token "*v" SPINE-JOIN 6 3) (box-immutable J-7-2)))
...
(define S-2-0 (split-node (token "*^" SPINE-SPLIT 2 0)
